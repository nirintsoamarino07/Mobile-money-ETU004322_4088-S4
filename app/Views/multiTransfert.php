...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="number"] { width: 100%; padding: 12px 16px; border: 1.5px solid #cbd5e0; border-radius: 10px; font-size: 1.05rem; outline: none; margin-bottom: 16px; }
        textarea { width: 100%; height: 100px; padding: 12px 16px; border: 1.5px solid #cbd5e0; border-radius: 10px; font-size: 1.05rem; outline: none; margin-bottom: 10px; resize: vertical; font-family: monospace; }
        input[type="number"]:focus, textarea:focus { border-color: #1a56db; }
        .helper-text { font-size: .8rem; color: #718096; margin-bottom: 24px; line-height: 1.4; }
...
<header>
    ...
    <nav>
        ...
        <a href="<?= site_url('multi-transfert') ?>" class="active">Envoi multiple</a>
...
<main>
    ...
    <div class="form-card">
        <h2>👥 Envoi Multiple Destinataires</h2>
        ...
        <form action="<?= site_url('multi-transfert') ?>" method="post">
            <?= csrf_field() ?>
            <label for="montant_total">Montant Total à Diviser (Ar)</label>
            <input type="number" id="montant_total" name="montant_total" required min="1" step="any" placeholder="Ex : 60000">

            <label for="destinataires">Numéros des Destinataires (un par ligne ou séparés par des virgules)</label>
            <textarea id="destinataires" name="destinataires" placeholder="Ex :&#10;0341111111&#10;0342222222" required></textarea>
            
            <div class="helper-text">
                ⚠️ <strong>Important :</strong> 
                Les fonds seront divisés à parts égales entre tous les destinataires valides.
                Tous les destinataires doivent appartenir à votre opérateur (de même préfixe).
            </div>

            <button type="submit">Confirmer l'envoi multiple</button>
        </form>
    </div>
</main>
</body>
